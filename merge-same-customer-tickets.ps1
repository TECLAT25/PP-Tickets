# merge-same-customer-tickets.ps1
# Fusiona automaticamente hilos nuevos de Gmail en un ticket existente
# del mismo cliente con el mismo asunto, salvo que ese ticket este
# cerrado (en cuyo caso se crea uno nuevo).
...
class GmailSyncEngine {
  ...
  synchronizeThread_(thread, mailbox, summary) {
    ...
    if (!ticket && customerEmail) {
      const mergeable = this.findMergeableTicket_(customerEmail, firstMessage.subject);
      if (mergeable) {
  ...
  /**
   * Finds an existing ticket for this customer with a matching (normalized)
   * subject, so a new Gmail thread about the same topic gets merged into it
   * instead of creating a duplicate ticket. Closed tickets are excluded —
   * a new thread about a closed topic still becomes its own ticket.
   * @param {string} customerEmail
   * @param {string} subject
   * @return {Object|null}
   * @private
   */
  findMergeableTicket_(customerEmail, subject) {
    ...
    const candidates = result.items.filter(function(candidate) {
      return candidate.status !== 'CLOSED' && normalize(candidate.subject) === targetSubject;
    });
...
function syncGmailFromMenu() {
  ...
  try {
    ...
    ui.alert(
      ...
      'Tickets fusionados (mismo cliente/asunto): ' + (result.mergedTickets || 0),
      ui.ButtonSet.OK
    );
  } catch (error) {
    const response = AppUtils.errorResponse(error);
...
(function () {
  ...
  function summaryText(result) {
    return 'Sincronización de Gmail completada: ' +
      ...
      (result.linkedTickets || 0) + ' tickets huérfanos vinculados, ' +
      (result.mergedTickets || 0) + ' tickets fusionados.';
  }
...
Select-String -Path src\gmail.gs -Pattern "findMergeableTicket_"
