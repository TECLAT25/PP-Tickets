...
class UiCustomerReadRepository {
  ...
  findForTicket(ticket) {
    ...
    return {
      ...
      firstName: String(UiSheetMapper.value(row, this.headers_, 'First Name') || ''),
      lastName: String(UiSheetMapper.value(row, this.headers_, 'Last Name') || ''),
      address: String(UiSheetMapper.value(row, this.headers_, 'Address') || ''),
      country: String(UiSheetMapper.value(row, this.headers_, 'Country') || ''),
      postalCode: String(UiSheetMapper.value(row, this.headers_, 'Postal Code') || '')
    };
  }
}
