...
class DraftReplyService {
   ...
   */
  createForTicket(ticketId, templateKey) {
    ...
    const body = this.renderBody_(ticket, templateKey || 'DEFAULT_SUPPORT_REPLY');
    const draft = latestMessage.createDraftReply(body, {
      htmlBody: this.renderHtmlBody_(ticket, templateKey || 'DEFAULT_SUPPORT_REPLY')
    });

    this.logger_.info('Draft reply created.', {ticketId: ticketId, templateKey: templateKey || 'DEFAULT_SUPPORT_REPLY'});
    return {
      ok: true,
      ticketId: ticketId,
      draftId: draft.getId(),
 ...
 * @return {{ok: boolean, ticketId: string, draftId: string, subject: string}}
 */
function createDraftForTicket(ticketId, templateKey) {
  return createDraftReplyService_().createForTicket(ticketId, templateKey || 'DEFAULT_SUPPORT_REPLY');
}
