  ...
  createForTicket(ticketId, templateKey, customBody) {
    ...
    if (!ticket.threadId) {
      if (!ticket.customerEmail) {
        throw new AppError('Ticket has no Gmail Thread ID and no customer email to draft to: ' + ticketId, 'DRAFT_THREAD_MISSING', {ticketId: ticketId});
      }
      const subject = ticket.subject || ('Ticket ' + ticketId);
      const draft = GmailApp.createDraft(ticket.customerEmail, subject, body, {
        htmlBody: htmlBody,
        from: this.settings_.get('SUPPORT_EMAIL', 'support@pocketpiano.com')
      });
      this.logger_.info('New draft created (no linked Gmail thread).', {
        ticketId: ticketId,
