# order-number-and-extraction-fixes.ps1
# Codigo postal ya no confunde anos, mejor deteccion de pais (dominio
# email como respaldo), y nuevo campo Numero de pedido (#00000).
...
'@
[System.IO.File]::WriteAllText((Join-Path $root "src\extraction.gs"), $v4, $enc)
Write-Host "  [OK]" -ForegroundColor Green
...
'@
[System.IO.File]::WriteAllText((Join-Path $root "html\TicketActions.html"), $v5, $enc)
Write-Host "  [OK]" -ForegroundColor Green
...
'@
[System.IO.File]::WriteAllText((Join-Path $root "html\GlobalSaveScripts.html"), $v6, $enc)
Write-Host "  [OK]" -ForegroundColor Green
...
Select-String -Path src\constants.gs -Pattern "Order Number"
Select-String -Path html\TicketActions.html -Pattern "ta-order-number"

Write-Host ""
Write-Host "Si salieron lineas arriba, ejecuta: npm test  y  npm run deploy" -ForegroundColor Cyan
Write-Host "Luego ejecuta install() una vez para crear la columna Order Number." -ForegroundColor Cyan
