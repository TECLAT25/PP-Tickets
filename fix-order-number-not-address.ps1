...
class MessageFieldExtractor {
  ...
  static get ORDER_NUMBER_PATTERN_() {
    return /(?:pedido|order|reference|referencia|n[uú]mero de pedido|order\s*number)\b[^\d#]{0,20}#?\s*\d{3,10}\b/i;
  }
  ...
  static extractPostalCode_(text) {
    const labelPattern = /(postal\s*code|c[oó]digo\s*postal|cod\.?\s*postal|c\.p\.?|code\s*postal|postleitzahl|plz|postnummer|kod\s*pocztowy|postcode|zip)\s*[:\-]?[ \t]*([A-Z0-9][A-Z0-9 \t-]{2,9})/i;
  ...
  static extractAddress_(text) {
    ...
    const addressWords = /\b(calle|avenida|avda|c\/|cl\.?|plaza|pza\.?|paseo|po\.?|camino|carretera|ctra\.?|ronda|passeig|pg\.?|pla[çc]a|carrer|bulevar|callej[oó]n|urbanizaci[oó]n|urb\.?|pol[ií]gono|apartado|street|st\.|road|rd\.|avenue|ave\.|lane|drive|boulevard|rue|via|viale|piazza|rua|ulica|ul\.)\b|[\wäöüß]*stra(?:ß|ss)e\b|[\wäöü]*straat\b|[\wäöü]*laan\b|[\wäö]*v[aä]gen\b|[\wäö]*gata\b/i;
    const labelPrefix = /^[\p{L}\s]{2,30}:\s*/u;
    const otherFieldLine = /(tel[eé]fono|phone|tel\.?|telefon|num[eé]ro|e-?mail)\s*[:\-]/i;
    const postalLikeNumber = /\b\d{4,6}\b/;
    ...
    // Fallback: no recognized street word, but a line that looks like
    // "<something> <house number>, <postal code> <city>" is still very
    // likely an address (e.g. customer wrote "Mayor 5, 28013 Madrid").
    // Skip date mentions and order/serial number mentions instead — those
    // are other known field types, not addresses.
    const dateLike = /\b(19|20)\d{2}\b|\b(enero|febrero|marzo|abril|mayo|junio|julio|agosto|septiembre|octubre|noviembre|diciembre|jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)\b/i;
    const otherKnownField = /(pedido|order|reference|referencia|n[uú]mero de serie|serial|n[uú]mero de tel[eé]fono)/i;
    for (let j = 0; j < lines.length; j += 1) {
      const raw = lines[j];
      if (raw.length > 6 && raw.length < 100 && postalLikeNumber.test(raw) && /\d.*[\p{L}]|[\p{L}].*\d/u.test(raw) && !otherFieldLine.test(raw) && !dateLike.test(raw) && !otherKnownField.test(raw)) {
        return raw.replace(labelPrefix, '');
      }
    }
    return '';
  }
