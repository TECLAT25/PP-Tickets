...
class MessageFieldExtractor {
  ...
  static extractAddress_(text) {
    ...
    // Skip anything that looks like a date mention instead.
    ...
    for (let j = 0; j < lines.length; j += 1) {
      const raw = lines[j];
      if (raw.length > 6 && raw.length < 100 && postalLikeNumber.test(raw) && /\d.*[\p{L}]|[\p{L}].*\d/u.test(raw) && !otherFieldLine.test(raw) && !dateLike.test(raw)) {
